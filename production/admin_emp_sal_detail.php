...
<head>
  ...
  include("head1.php");
	...
	{
		echo "post is :".$post;
		$result=mysqli_query($con, "select * from emp_salary_detail where post_code='$post'");
		$total=mysqli_num_rows($result);
		if($total>0)
		{
		?> <script>alert("data already saved");</script><?php	
		}
		else
		{
		 mysqli_query($con,"insert into emp_salary_detail values('','$dept','$post','$b_sal','$house','$medical','$dearness','$increment','$incentive','$outdoor','$transport','$total_ern','$loss_wage','$pro_fund','$T_leave','$tax','$other1','$total_dedu','$n_pay')");
		}
...
document.getElementById("t_earn").value=b_sal+(b_sal)*h_allow/100+(b_sal)*m_allow/100+(b_sal)*dearness/100+(b_sal)*increment/100+(b_sal)*incentive/100+(b_sal)*outdoor/100+(b_sal)*transport/100;
document.getElementById("t_dedu").value=(b_sal)*l_wage/100+(b_sal)*p_fund/100+(b_sal)*tax/100+(b_sal)*other/100;
...
function check_status()
{
	...
	var b=document.getElementById('post').value;
	 $.ajax
	  ({
		  type:"POST",
            ...
            <div class="col-md-12 col-sm-12 col-xs-12">
              <div class="x_panel">
                ...
                <div class="x_content">
				  ...
				  <select name="dept" class="form-control" id="dept" onchange="get_dept(this.value);">
				 ...
				 <br> <span id="s"></span>
				  ...
				  <div class="col-sm-3">
				 <label>Post</label><br>
				  <select name="post" id="post" class="form-control" onchange="check_status();">
				  </select>
				  </div>
				  <div class="col-sm-1">
				</div>
				  ...
				  <input type="number" name="b_sal" class="form-control" id="b_sal" required value="0" onblur="basic_add();" onclick="check_status();">
